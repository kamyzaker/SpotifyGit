...
async def get_user_spotify_status(
    conn: AsyncConnection,
    *,
    user_id: int,
) -> tuple[Any, ...] | None:
    async with conn.cursor() as cursor:
        ...
        row = await data.fetchone()
    if row:
        logger.info("The user with `user_id`=%s has the spotify_status %s", user_id, row[0])
    else:
...
async def change_auth_on_spotify_status(
    conn: AsyncConnection,
    *,
    auth_on_spotify: bool,
    user_id: int,
) -> None:
    async with conn.cursor() as cursor:
        await cursor.execute(
            query="""
                UPDATE users
                SET auth_on_spotify = %s
                WHERE user_id = %s;
            """,
            params=(auth_on_spotify, user_id)
        )
    logger.info("Updated `auth_on_spotify` status to `%s` for user %d", auth_on_spotify, user_id)
